...
int sqlveriyaz2(wstring sorgu, wstring baglantiyol)
{
...
void sqlbaglantiac(wstring baglantiyol) {
...
void sql_veriyaz(wstring sorgu) {
...
int sql_veriokusayisal(wstring sorgu) {
...
int resimidolustur() {
	...
	wstring sorgu = L"SELECT MAX(Resimid)  FROM Kayit ";
	int donus = sql_veriokusayisal(sorgu);
	donus = donus + 1;
	cout << "Donus Resim idsi= " << donus;
	return donus;

}


void sqlbaglantikapat() {
	SQLFreeHandle(SQL_HANDLE_STMT, sqlStmtHandle);
	SQLDisconnect(sqlConnHandle);
	SQLFreeHandle(SQL_HANDLE_DBC, sqlConnHandle);
	SQLFreeHandle(SQL_HANDLE_ENV, sqlEnvHandle);
